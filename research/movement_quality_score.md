...
6. **Insufficient evidence guard (v1.7):** When overall signal completeness drops below 50%, `compute_gait_summary` sets `movement_quality_score` to NaN instead of a misleading mid-range value. The `mqs_sufficient_evidence` flag (0.0 or 1.0) indicates whether the score is reliable. Per-domain scores are still computed for diagnostic purposes but the composite is withheld. This prevents sparse/failed pose estimation from producing plausible-looking scores. The dashboard renders "—" instead of a numeric MQS when evidence is insufficient.

7. **Video MQS confidence degradation (v1.2, v1.5, v1.6):** When MQS is computed from video-derived poses and `pose_metadata` is provided to `compute_gait_summary`, the summary includes `mqs_confidence_factor` and `movement_quality_score_weighted` (= raw × CF). CF = observed_fraction × mean_confidence × (1 − interpolation_fraction × 0.5). The interpolation penalty (v1.6) accounts for linearly interpolated missing frames that inflate signal completeness without adding real data. The unscaled `movement_quality_score` is always preserved. Synthetic data (no pose metadata) receives CF = 1.0 and no weighted key is emitted. Video frontal-plane SI (pelvis obliquity, trunk lean) is automatically skipped when the left/right signals share identity — a structural guard against invalid symmetry computation from the single global pelvis/trunk signal extracted by monocular pose estimation.
